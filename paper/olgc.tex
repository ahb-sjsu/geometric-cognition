...
is asked to predict. A pilot of the first phase then shows that this condition
has a second edge. An identification accurate enough to predict unseen
comparisons almost perfectly makes the prediction it licenses close to a
consequence of the fit, so content is not monotone in identification accuracy
and we can bound neither end. What survives is that identification is the step
that fails in practice, since two of three candidate evaluators could not be
calibrated at all. We restate the five predictions of the programme as
...
metric says the pair's rank-$k$ order disagrees with its full-budget order.
Reversal is then predicted for that class and not for the class whose difference
lies in the retained subspace. On the pinned evaluator the calibration predicted
held-out comparisons at $0.985$, and the graded contrast came out at $0.968$
against $0.000$. The separation is large, and most of it is entailed. An
evaluator that the fitted metric describes almost exactly will reverse almost
exactly the pairs the fitted metric says it will reverse, so at that accuracy
the reversal test is close to a corollary of the calibration rather than an
independent check on the budget.

This is not an artifact of one design. Any test of the form \emph{identify the
components, then predict a choice from them} inherits it, because the prediction
is a function of the identified components and its informativeness falls as the
identification error falls toward zero. Content is therefore not monotone in
identification accuracy. It is low at both ends and the maximum is somewhere in
between, and we can locate neither end.
...
An evaluator can fail to be a quadratic evaluation object & measured, not sealed & Phase I instrument search \cite{c1}. Two of three candidate language-model evaluators could not be calibrated, ordering by presentation position at 0.8175 and 0.700. The third calibrated and held out at 0.985 \\
...
that load adds noise. Open~\ref{open:band} bounds what a confirmation of it is
worth, because the class assignment is made by the same fitted metric that
...
No gate in this paper has been sealed. The Phase I pilot and the instrument
search are design work and grade nothing, and the one row of
