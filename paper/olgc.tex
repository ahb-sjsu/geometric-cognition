...
algebraic identity in the evaluator's accuracy and measures nothing else, so
...
label is therefore the prediction, and the consequence is not approximate. Write
$p$ for the probability that a graded verdict agrees with the fitted metric. A
trading pair reverses when the evaluator agrees with the fit at both budgets or
errs at both, and a within-subspace pair reverses when it errs at exactly one,
so
\[
R_T = p^2 + (1-p)^2, \qquad R_W = 2p(1-p), \qquad
R_T - R_W = (2p-1)^2 .
\]
The contrast the design grades is a function of the evaluator's per-comparison
accuracy and of nothing else. On the pinned evaluator the observed contrast of
$0.968$ against $0.000$ implies $p = 0.992$, against a measured held-out
accuracy of $0.985$. No part of the separation is left for the budget
manipulation to explain, and the pass bars are the same quantity restated: the
registered margin is met exactly when $p$ exceeds $0.848$.
...
The design is being repaired before it is sealed. The Phase I pilot and the
