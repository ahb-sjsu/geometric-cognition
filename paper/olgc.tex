...
narrowing it, and a load the instrument does survive degrades the reliability of
a verdict before it degrades the geometry behind it. We restate the five
...
language-model evaluator. None produced a resolution effect. The failures are
reported here because they are more informative than the designs would have been
had they worked, and because two of the three failure modes are available to any
experiment of this shape.
...
of $0.8175$ and $0.700$. Deliberation is what makes this class of evaluator
...
This manipulation preserved the instrument. Every level passed the admissibility
gate, first-position rates stayed within $0.061$ of each cell's answer-key rate,
and reasoning rose from $398$ to $918$ tokens, so the load demonstrably landed.
What it did not produce was a resolution effect. Scoring every pair at every
load, with a swap-inconsistent verdict counted as uninformative rather than
dropped, the trading-pair score was flat across the ladder at a correlation
against load of $-0.28$, and its confidence intervals never came near one half
from above, let alone crossed it. Within-subspace scores fell, from $0.977$ to
$0.906$, because that class began at the ceiling and an uninformative verdict can
only move it down. The class gap narrowed, and it narrowed because the control
descended toward the trading class rather than because the trading class
degraded. A budget predicts the opposite shape.

What load did move was reliability. Swap consistency fell from $0.984$ to $0.844$
in the control class and to $0.797$ in the trading class. The pattern across all
three designs is the same: a resource manipulation applied to this evaluator
reaches the instrument before it reaches the geometry.
...
does not describe this substrate. Until a manipulation degrades resolution while
